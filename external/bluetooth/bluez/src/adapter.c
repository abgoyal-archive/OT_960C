...
#ifdef BT_ALT_STACK
uint32_t btd_adapter_get_class(struct btd_adapter *adapter, uint8_t* major,
							uint8_t* minor) 
{
								...
								void *data)
{
	...
	if (dbus_message_get_args(msg, NULL, DBUS_TYPE_OBJECT_PATH, &path,
						DBUS_TYPE_INVALID) == FALSE)
    {   
        printf("***** remove_device : invalid args\r\n");
		return invalid_args(msg);
    }
	...
	if (!l)
    {   
        printf("***** remove_device : device does not exist\r\n");
		return g_dbus_create_error(msg,
    ...
    {   
        printf("***** remove_device : device is temporary or busy\r\n");
		return g_dbus_create_error(msg,
				ERROR_INTERFACE ".DoesNotExist",
	...
	if (!device_is_connected(device)) {
        printf("******** remove_device: Device is not connected \r\n");
		adapter_remove_device(conn, adapter, device, TRUE);
		return dbus_message_new_method_return(msg);
	}
    printf("******** remove_device:Device is connected - requesting disconnect\r\n");
...
// BRCM enhancement start - Btld specific oob support
...
void dtun_sig_dm_local_oob_keys(tDTUN_DEVICE_SIGNAL *msg)
{
    ...
    // TODO add handling oob keys when NFC is ready

}
// BRCM enhancement end - Btld specific oob support
...
static void emit_cbc_level(const char *path, const char *address, uint32_t batterylevel)
{
    DBusMessage *signal;
    DBusMessageIter iter;
    va_list var_args;
    ...
    dbus_message_append_args(signal, DBUS_TYPE_STRING, &address, DBUS_TYPE_UINT32, &batterylevel, DBUS_TYPE_INVALID);
    g_dbus_send_message(connection, signal);
}

void adapter_emit_cbc_level(struct btd_adapter *adapter, const bdaddr_t *peer, uint32_t batterylevel) 
{
    char peer_addr[18];
    const char *paddr = peer_addr;
    ba2str(peer, peer_addr);
    info("sending cbc level for address %s battery level %d", paddr, batterylevel);
    emit_cbc_level(adapter->path, paddr, batterylevel);
}
...
void adapter_emit_wbs_config(struct btd_adapter *adapter, uint8_t wbs)
{
    ...
    info("Sending wbs config %d", wbs_config);
				...
				struct remote_dev_info *dev, uint8_t *eir_data)
{
	...
	info("%s: got dev_type = 0x%x 0x%x", __FUNCTION__, dev->dev_type, dev_type);
        /* Extract UUIDs from extended inquiry response if any*/
        if ( dev->dev_type != DEVICE_TYPE_BLE ) 
        {
            if (eir_data != NULL)
            {
...
#ifdef BT_ALT_STACK
				name_status_t name_status, uint8_t dev_type, uint8_t * eir_data)
#else 
...
#ifdef BT_ALT_STACK
    info("%s: got dev_type = 0x%x", __FUNCTION__, dev_type);
#endif
