...
void agent_free(struct agent *agent)
{
	...
	if (agent->request) {
		...
		default:
			cb = agent->request->cb;
			cb(agent, &err, agent->request->user_data,0);
		}
				...
				const char *uuid)
{
    ...
    gboolean bTemporary = device_is_temporary(device);
