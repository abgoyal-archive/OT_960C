...
#endif
	info("errno before calling connect is %d", errno);
	err = connect(sock, (struct sockaddr *) &addr, sizeof(addr));
	info("errno after calling connect is %d", errno);
	info ("return value from connect is %d", err);
					...
					struct set_opts *opts, GError **err)
{
	...
	case BT_IO_L2RAW:
		...
		if (server && l2cap_bind(sock, &opts->src,
