...
int hcid_dbus_request_pin(int dev, bdaddr_t *sba, struct hci_conn_info *ci, uint8_t is_le_only)
{
	struct btd_adapter *adapter;
	struct btd_device *device;
	...
	return device_request_authentication(device, AUTH_TYPE_PINCODE, 0,
								pincode_cb,is_le_only);
}

static int confirm_reply(struct btd_adapter *adapter,
				struct btd_device *device, gboolean success, uint8_t is_le_only)
{
	int dd;
	...
	dtun_ssp_confirm_reply(&bdaddr.b, success, is_le_only);
#else
...
#ifdef BT_ALT_STACK
static void confirm_cb(struct agent *agent, DBusError *err, void *user_data, uint8_t is_le_only)
#else
...
#endif
{
	...
	confirm_reply(adapter, device, success, is_le_only);
}
...
#ifdef BT_ALT_STACK
int hcid_dbus_user_confirm(bdaddr_t *sba, bdaddr_t *dba, uint32_t passkey, uint8_t is_le_only)
#else
...
#endif
{
	...
	if (!get_adapter_and_device(sba, dba, &adapter, &device, TRUE))
		return -ENODEV;
#ifdef BT_ALT_STACK
    /* for BLE link, go for auth right away. For BR/EDR, we will check if we need to auto-confirm */
    if (is_le_only)
        return device_request_authentication(device, AUTH_TYPE_CONFIRM,
                        passkey, confirm_cb,is_le_only);
#endif
	...
	info("remote IO capabilities are 0x%02x", rem_cap);
	info("remote authentication requirement is 0x%02x", rem_auth);
	...
	if ((loc_auth == 0xff || !(loc_auth & 0x01) || rem_cap == 0x03) &&
		...
		info("auto accept of confirmation");

		/* Wait 5 milliseconds before doing auto-accept */
		usleep(5000);

#ifdef BT_ALT_STACK
        if (confirm_reply(adapter, device, TRUE, is_le_only) < 0)
            return -EIO;
...
fail:
	return confirm_reply(adapter, device, FALSE, is_le_only);
}
...
#ifdef BT_ALT_STACK
				int8_t rssi, uint8_t *data, uint8_t dev_type)
#else
...
#ifdef BT_ALT_STACK
						NAME_NOT_REQUIRED, dev_type, data);
#else
						NAME_NOT_REQUIRED);
#endif
		return;
...
#ifdef BT_ALT_STACK
					legacy, name_status, dev_type, data);
#else
					legacy, name_status);
#endif
...
void hcid_dbus_notify_cbc_level(bdaddr_t *local, bdaddr_t *peer, uint32_t batterylevel)  
{
	struct btd_adapter *adapter = manager_find_adapter(local);	
	if (! adapter) {
		error("Unable to find matching adapter");
	...
	adapter_emit_cbc_level(adapter, peer, batterylevel);
}

void hcid_dbus_send_wbs_config (bdaddr_t *local, uint8_t wbs)
{
	struct btd_adapter *adapter = manager_find_adapter(local);
	if (! adapter) {
		error("Unable to find matching adapter");
				...
				int pin_length, uint8_t old_key_type)
{
	 ...
	 * 2. this is an authentication combination link key
	...
	if (new_key_type == 0x03) {
		...
		error("storing link key of type 0x%02x", key_type);
