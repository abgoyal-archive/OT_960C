...
#ifdef BT_ALT_STACK
        //BLE: return device type
	if (read_device_type(&src,&device->bdaddr, &dev_type) != 0)
	{
	    info("%s: unable to read device type.... ",__FUNCTION__);
	    dev_type=0;
        }
	info("%s: returning dev_type = 0x%x", __FUNCTION__, dev_type);
					...
					DBusMessage *msg, void *user_data)
{
    ...
    //Although, btld don't care about this reply. we still need reply it as success immediately to clean-up the reply recorder in dbus daemon
    debug("replying DiscoverServices as success"); 
    boolean success = 1;  //<<< morris added
    DBusMessage* reply = dbus_message_new_method_return(msg);
    dbus_message_append_args(reply, DBUS_TYPE_BOOLEAN, &success, DBUS_TYPE_INVALID); //<<<morris added
...
void discover_services_reply(int err,sdp_list_t * recs) {
	...
	if (!reply) {
	  error("Unable to create reply message...Exitting...");
	  return;
	}
...
{
    printf("*********** Device handle = 0x%x", device->handle);
...
void device_remove(struct btd_device *device, gboolean remove_stored)
{
    ...
    printf("*****removing device agent = 0x%x", device->agent);
	if (device->agent)
		agent_free(device->agent);

    printf("*****removing device bonding = 0x%x", device->bonding);

	if (device->bonding)
		device_cancel_bonding(device, HCI_OE_USER_ENDED_CONNECTION);

    printf("*****removing device browse = 0x%x", device->browse);

	if (device->browse)
		browse_request_cancel(device->browse);

    printf("*****removing device handle = 0x%x", device->handle);
...
void device_bonding_complete(struct btd_device *device, uint8_t status)
{
	...
	// set paired property since status is success
...
static void confirm_cb(struct agent *agent, DBusError *err, void *data)
{
	...
	((agent_cb) auth->cb)(agent, err, device, auth->is_le_only);
...
static void pairing_consent_cb(struct agent *agent, DBusError *err, void *data)
{
	...
	((agent_cb) auth->cb)(agent, err, device,0);
...
int device_request_authentication(struct btd_device *device, auth_type_t type,
                uint32_t passkey, void *cb, uint8_t is_le_only)
#else
int device_request_authentication(struct btd_device *device, auth_type_t type,
