...
#ifdef BT_ALT_STACK
void pin_code_request(int dev, bdaddr_t *sba, bdaddr_t *dba, uint8_t is_le_only)
#else
...
#endif
{
	...
	if (pinlen > 0) {
		...
		pr.pin_len = pinlen;
#ifdef BT_ALT_STACK
		dtun_pin_reply(DTUN_METHOD_DM_PIN_REPLY, &pr, is_le_only);
#else
		...
		if (hcid_dbus_request_pin(dev, sba, ci, is_le_only) < 0)
			goto reject;
	}
...
#ifdef BT_ALT_STACK
    dtun_pin_reply(DTUN_METHOD_DM_PIN_NEG_REPLY, &pr, is_le_only);
#else
...
static inline void inquiry_result(int dev, bdaddr_t *sba, int plen, void *ptr)
{
	...
	for (i = 0; i < num; i++) {
		...
		hcid_dbus_inquiry_result(sba, &info->bdaddr, class, 0, NULL, 0);
								...
								gpointer data)
{
	...
	case EVT_PIN_CODE_REQ:
		pin_code_request(dev, &di->bdaddr, (bdaddr_t *) ptr, 0);
		break;
