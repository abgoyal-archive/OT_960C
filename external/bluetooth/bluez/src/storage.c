...
int write_remote_eir(bdaddr_t *local, bdaddr_t *peer, uint8_t *data)
{
	...
	create_filename(filename, PATH_MAX, local, "eir");
        debug("writing eir to path %s", filename);
	create_file(filename, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
...
int write_device_type(bdaddr_t *local, bdaddr_t *peer, uint8_t dev_type) 
{
    char filename[PATH_MAX + 1], addr[18], str[9];

    create_filename(filename, PATH_MAX, local, "devicetypes");
    create_file(filename, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);

    ba2str(peer, addr);
    sprintf(str, "0x%02x", dev_type);

    return textfile_put(filename, addr, str);

}

int read_device_type(bdaddr_t *local, bdaddr_t *peer, uint32_t * dev_type )
{
    char filename[PATH_MAX + 1], addr[18], *str;
