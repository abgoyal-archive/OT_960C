...
#if 1	// by sinclair.lee 20120525  force e2fsck to fix userdata partition all the time. same effect if cmdline is given not by -p but by -y
	else
	{
		char *p = "/dev/block/mmcblk0p19";
		if(strncmp(p, ctx->filesystem_name,strlen(p)) == 0 && (ctx->options & E2F_OPT_PREEN) != 0)
		{
			ctx->options &= ~E2F_OPT_PREEN;
			ctx->options |= E2F_OPT_YES;
		}
