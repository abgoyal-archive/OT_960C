...
#if 1	// 20120406 by sinclair.lee In recovery mode, do not initiate rmt_storage. rmt_storage is not running in recovery mode
...
#if 1	// 20120313 added by sinclair.lee for NVBACKUP FEATURE_M7_FW_MEM_SUPERBLOCK
/* NVBACKUP */
#define RAMFS_NBK_STORAGE_ID		0x004E424B
#endif
...
#if 1	// 20120313 added by sinclair.lee for NVBACKUP FEATURE_M7_FW_MEM_SUPERBLOCK
	ramfs_entry = &ramfs_table->ramfs_entry[2]; // only 0, 1 are used in msm8x55.
	{
		// ramfs_entry is &ramfs_table->ramfs_entry[2] which is an unused value now
		// so no problem at all
		// virt_to_phy() function not work properly. so, msm8x5x/7x30 dedicated code inserted
		// ramfs_entry->base_addr should be 4KB aligned (2^12), because of mmap()
		// physical address : 0x118780, virtual address in kernel : 0xFA118780, user space : ?????
		void* virt_base_addr = (void*) smem_get_entry(SMEM_ID_VENDOR0, &ramfs_entry->size);

		// 4KB align
		unsigned long phys_base_addr = ((unsigned long)virt_base_addr & 0x00FFF000);
		ramfs_entry->size += 0x1000;

		if(!virt_base_addr)
		{
			pr_err("%s: No NVBACKUP_FS(SMEM_ID_VENDOR0) table in SMEM\n", __func__);
			return -ENOENT;
		}
		ramfs_entry->base_addr = (uint32_t) phys_base_addr;
		pr_info("%s: NVBACKUP_FS entry: addr = 0x%08x, size = 0x%08x\n",
			__func__, ramfs_entry->base_addr, ramfs_entry->size);

		ramfs_entry->client_id = RAMFS_NBK_STORAGE_ID;
		ret = rmt_storage_add_shrd_mem(ramfs_entry->client_id,
					       ramfs_entry->base_addr,
					       ramfs_entry->size,
					       (void*)((unsigned long)virt_base_addr & 0xFFFFF000),
					       ramfs_entry,
					       srv);
		if (ret) {
			pr_err("%s: Error (%d) adding shared mem..\n",
...
static uint32_t rmt_storage_get_sid(const char *path)
{
	...
	if (!strncmp(path, "/boot/modem_fsg", MAX_PATH_NAME))
		return RAMFS_MODEMSTORAGE_ID;
#if 1	// added by sinclair.lee for NVBACKUP  FEATURE_M7_FW_MEM_SUPERBLOCK
	if (!strncmp(path, "/boot/modem_nbk", MAX_PATH_NAME))
		return RAMFS_NBK_STORAGE_ID;
#endif
...
#if 1	// 20120406 by sinclair.lee In recovery mode, do not initiate rmt_storage. rmt_storage is not running in recovery mode
	// rmt_storage is handled by this file and vendor/qcom/proprietary/remotefs/*
	// In recovery mode, other half process dees not run and it will cause CP watchdog fail during efs_sync()

	int fd=sys_open("/sbin/recovery", O_RDONLY, 0);
	if(fd < 0)
		printk(KERN_INFO "%s: start rmt_storage now\n", __func__);
	else
	{
		printk(KERN_INFO "%s: skip rmt_storage in recovery mode\n", __func__);
		sys_close(fd);
		return -EPERM;
	}
#endif
