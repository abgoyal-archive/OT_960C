...
#define DEFAULT_SERIAL_NUMBER	//	Teddy 2012-05-01
#ifdef DEFAULT_SERIAL_NUMBER	//	Teddy 2012-05-01
int msm_hsusb_check_serial(void)
{
	int rc = 0;
	struct hsusb_start_req {
		struct rpc_request_hdr hdr;
	...
	rc = msm_rpc_call_reply(usb_ep, 80, &req, sizeof(req), &rep, sizeof(rep), 5 * HZ);
	if (rc < 0)
		pr_err("%s: rpc call failed! error: %d\n" ,
			__func__, rc);
	else
	...
	pr_info("%s : rc = %d, return=%d\n",__func__, rc, rep.result );
	return rep.result;
}
EXPORT_SYMBOL(msm_hsusb_check_serial);
#endif	//	DEFAULT_SERIAL_NUMBER
