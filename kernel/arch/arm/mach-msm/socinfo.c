...
#if 1	//	Teddy 2012-01-03
static	uint32_t	hw_rev_id = 0;
static	uint32_t	hw_product_id = 0;
static	int			hw_check_hw_id = 0;

static uint GPIO_HW_ID[] =
{
	102,
	103,
	104,
...
static HW_ID_MapType gpio_to_platform[] =
{
	{0x0001, (uint32_t)HW_ID_REV_V1A},
	{0x0000, (uint32_t)HW_ID_REV_V2A},
	{0x0002, (uint32_t)HW_ID_REV_V3A},
	{0x0003, (uint32_t)HW_ID_REV_V4A},
	{0x0004, (uint32_t)HW_ID_REV_V5A},
	{0x0005, (uint32_t)HW_ID_REV_V6A},
	{0x0006, (uint32_t)HW_ID_REV_V7A},
	{0x0007, (uint32_t)HW_ID_REV_V8A},
	{0x0008, (uint32_t)HW_ID_REV_V9A},
	{0x000F, (uint32_t)HW_ID_REV_MAX},
};

static void check_hw_id( void )
{
	uint	read_value = 0, read_gpio = 0;
	uint	hw_rev_value = 0, hw_product_value = 0;
	int		index, count = sizeof(GPIO_HW_ID)/sizeof(GPIO_HW_ID[0]);

	if( hw_check_hw_id )
	{
		pr_info("%s1: read_hwid=0x%X, hw_rev_id=%d, hw_product_id=%d\n", __func__, read_value, hw_rev_id, hw_product_id);
		return;
	}

	for( index = 0; index < count;index++)
		gpio_tlmm_config(
			GPIO_CFG(GPIO_HW_ID[index], 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA),
			GPIO_CFG_ENABLE);

	for( index = 0; index < count;index++)
	{
		read_gpio = (uint)gpio_get_value(GPIO_HW_ID[index]);
		read_value |= (read_gpio << index);
	}

	hw_rev_value = (read_value & 0x000F);
	hw_product_value = (read_value >> 4);
	pr_info("%s2: read_hwid=0x%X, hw_rev=%d, hw_product=%d\n", __func__, read_value, hw_rev_value, hw_product_value);

	for( index = 0; index < sizeof(gpio_to_platform)/sizeof(gpio_to_platform[0]);index++)
	{
		if( gpio_to_platform[index].value == hw_rev_value )
			hw_rev_id = gpio_to_platform[index].platform_version;
	}

	hw_product_id = (uint32_t)hw_product_value;
...
{
	check_hw_id();
	pr_info("%s: hw_rev_id=0x%X \n", __func__, hw_rev_id);  // kykim log 
...
int get_npi_mode( void )
{
	gpio_tlmm_config(GPIO_CFG(101, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA), GPIO_CFG_ENABLE);

	return ((uint)gpio_get_value(101)?0:1);
}
#endif
