			   ...
			   unsigned int iocmd, unsigned long ioarg)
{
...
#if 1	//M7-EZ-YuTaeKeun : added for slate - #start
	for (i = 0; i < diag_max_registration; i++) {
		...
		{
			if(driver->table[i].cmd_code == 0xFF && driver->table[i].subsys_id == 0xFF && driver->table[i].cmd_code_lo == 0xF6 && driver->table[i].cmd_code_hi == 0xF6)
			{
				if(pkt_params->params->cmd_code == 0xFF && pkt_params->params->subsys_id == 0xFF && pkt_params->params->cmd_code_lo  == 0xF6 && pkt_params->params->cmd_code_hi == 0xF6)
				{
					driver->table[i].process_id = current->tgid;
					success = 1;
					count_entries++;
			...
			else	if(driver->table[i].cmd_code == 0xFF && driver->table[i].subsys_id == 0xFF && driver->table[i].cmd_code_lo == 0xAA && driver->table[i].cmd_code_hi == 0xAA)
				{
					if(pkt_params->params->cmd_code == 0xFF && pkt_params->params->subsys_id == 0xFF && pkt_params->params->cmd_code_lo  == 0xAA && pkt_params->params->cmd_code_hi == 0xAA)
					{
						driver->table[i].process_id = current->tgid;
						success = 1;
						count_entries++;
			...
			else	if(driver->table[i].cmd_code == 0xFF && driver->table[i].subsys_id == 0xFF && driver->table[i].cmd_code_lo == 0xBB && driver->table[i].cmd_code_hi == 0xBB)
				{
					if(pkt_params->params->cmd_code == 0xFF && pkt_params->params->subsys_id == 0xFF && pkt_params->params->cmd_code_lo  == 0xBB && pkt_params->params->cmd_code_hi == 0xBB)
					{
						driver->table[i].process_id = current->tgid;
						success = 1;
						count_entries++;
	...
	if (i < diag_threshold_registration) {
		...
		if (i < diag_threshold_registration) {
			...
			pr_err("Max size reached, Pkt Registration failed for"
						" Process %d", current->tgid);
#endif	//M7-EZ-YuTaeKeun : added for slate - #end
