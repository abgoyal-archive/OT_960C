...
#define FW_UPGRADE_ENABLE /*Enable upgrade TP Firmware through i2c bus*/
...
#define KEY_Y_BOTTOM		870
#define KEY_HOME_XL		20
#define KEY_HOME_XR		100
#define KEY_MENU_XL		152
#define KEY_MENU_XR		212
#define KEY_BACK_XL		272
#define KEY_BACK_XR		332
...
/* Use virtual key mechanism of ANDROID */
#define VULCAN_AND_VIRTUAL_KEY
...
extern int ft_tsp_power( int on );	//	Teddy 2012-02-06

static struct i2c_adapter *this_adapter;
static struct i2c_client *this_client;


#ifdef VULCAN_AND_VIRTUAL_KEY

/*virtual key support */
static ssize_t ft5306_vkeys_show(struct kobject *kobj,
			struct kobj_attribute *attr, char *buf)
{
#if 1	// by sinclair.lee 20120321  adjust the position
	return sprintf(buf,
	__stringify(EV_KEY) ":" __stringify(KEY_HOME) ":76:845:80:50"
	":" __stringify(EV_KEY) ":" __stringify(KEY_MENU) ":195:845:80:50"
	":" __stringify(EV_KEY) ":" __stringify(KEY_BACK) ":314:845:80:50"
	":" __stringify(EV_KEY) ":" __stringify(KEY_SEARCH) ":433:845:80:50"
	"\n");
#else
	return sprintf(buf,
	__stringify(EV_KEY) ":" __stringify(KEY_HOME) ":40:845:80:50"
	":" __stringify(EV_KEY) ":" __stringify(KEY_MENU) ":182:845:60:50"
	":" __stringify(EV_KEY) ":" __stringify(KEY_BACK) ":302:845:60:50"
	":" __stringify(EV_KEY) ":" __stringify(KEY_SEARCH) ":418:845:80:50"
	"\n");
#endif
}

static struct kobj_attribute ft5306_vkeys_attr = {
	.attr = {
		.mode = S_IRUGO,
	},
	.show = &ft5306_vkeys_show,
};

static struct attribute *ft5306_properties_attrs[] = {
	&ft5306_vkeys_attr.attr,
	NULL
};
...
static int ft5306_virtual_key_init(void)
{
	int rc = -EINVAL;

	/* virtual keys */
	ft5306_vkeys_attr.attr.name = "virtualkeys.ft5306_ts";
...
void ft5306_reset_func_gpio(void)
{
	gpio_set_value_cansleep(PM8058_GPIO_PM_TO_SYS(TS_RESET_PMIC_GPIO), 1);
	msleep(5);
	gpio_set_value_cansleep(PM8058_GPIO_PM_TO_SYS(TS_RESET_PMIC_GPIO), 0);
...
#endif
	ft_tsp_power(0);	//	Teddy 2012-02-06
	return 0;
}

static u8 fts_register_write(u8 e_reg_name, u8 b_data);
u32 TSP_Charger_Type;
static int ft5306_state = 0;	// by sinclair.lee 20121019 to avoid crash on no TSP
extern void test_lcd_backlight(int value);
void ft_5306_anti_noise_enable(void)
{
	int ret;
	u8 value;

	if(ft5306_state < 1)	// by sinclair.lee 20121019 to avoid crash on no TSP
		return;

	value = TSP_Charger_Type ? 1 : 0;

	ret = fts_register_write(0x8b, value);
	printk(KERN_ERR "leesh fts_register_write = ret = %x.\n", ret);

	ret = ft5306_i2c_read_value(this_client, 0x8b, &value, 1);
	printk(KERN_ERR "leesh ft5306_i2c_read_value = ret = %x, value = %d.\n", ret, value);
}
...
static int ft5306_resume(struct i2c_client *client)
{
	...
	//ft_tsp_power(1);	//	Teddy 2012-02-06
#ifdef WAKE_ENABLE
	gpio_tlmm_config(GPIO_CFG(TS_WAKE_GPIO, 0, GPIO_CFG_OUTPUT,
							GPIO_CFG_PULL_UP, GPIO_CFG_8MA), GPIO_CFG_ENABLE);
	msleep(1);
...
static char const *const touch_class_name = "touchscreen";
static char const *const ft5306_device_name = "ft5306";
static char const *const device_link_name = "i2c";
static dev_t const ft5306_device_dev_t = MKDEV(INPUT_MAJOR, 240);
static u8 ft5306_firmware_ver=0;
...
static ssize_t ft5306_firmware_version_show(
	struct device *dev, struct device_attribute *attr, char *buf)
{
	printk(KERN_ERR "leesh ft5306_firmware_ver = %x\n", ft5306_firmware_ver);
	return sprintf(buf, "V%02x\n", ft5306_firmware_ver);
}

static struct device_attribute ft5306_attributes[] = {

	__ATTR(touch_firmware_version,   0775, ft5306_firmware_version_show, NULL),
	__ATTR_NULL,
};

static int create_sysfs_interfaces_touch(struct ft5306_data *ft5306)
{
	int err;
	...
	ft5306->touchscreen = class_create(THIS_MODULE, touch_class_name);
	if (IS_ERR(ft5306->touchscreen)) {
		err = PTR_ERR(ft5306->touchscreen);
		goto exit_class_create_failed;
	}

	ft5306->class_dev = device_create(
						ft5306->touchscreen,
						NULL,
						ft5306_device_dev_t,
						ft5306,
						ft5306_device_name);
	if (IS_ERR(ft5306->class_dev)) {
		err = PTR_ERR(ft5306->class_dev);
		goto exit_class_device_create_failed;
	}

	err = sysfs_create_link(
			&ft5306->class_dev->kobj,
			&ft5306->client->dev.kobj,
			device_link_name);
	if (0 > err)
...
exit_device_attributes_create_failed:
	sysfs_remove_link(&ft5306->class_dev->kobj, device_link_name);
exit_sysfs_create_link_failed:
	device_destroy(ft5306->touchscreen, ft5306_device_dev_t);
exit_class_device_create_failed:
	ft5306->class_dev = NULL;
	class_destroy(ft5306->touchscreen);
exit_class_create_failed:
	ft5306->touchscreen = NULL;
	return err;
}

static void remove_sysfs_interfaces_touch(struct ft5306_data *ft5306)
{
	if (NULL == ft5306)
		return;

	if (NULL != ft5306->class_dev) {
		...
		const struct i2c_device_id *id)
{
	...
	create_sysfs_interfaces_touch(data);
...
static int __devexit ft5306_remove(struct i2c_client *client)
{
	...
	remove_sysfs_interfaces_touch(data);
...
{
//#include "Vulcan_V0d_20120411_app.h"
//#include "Vulcan_V0e_20120502_app.h"
//#include "Vulcan_V0f_20120911_app.h"
#include "Vulcan_V10_20120913_app.h"
};
...
static int __init ft5306_ts_init(void)
{
	...
	ft5306_state = 1;	// by sinclair.lee 20121019 to avoid crash on no TSP

	ft5306_i2c_read_value(this_client, CHIP_ID_ADDR, &chip_id, 1);
	msleep(1);
	if(ft5306_i2c_read_value(this_client, CHIP_ID_ADDR, &chip_id, 1)) {
		...
		for (i = 0; i < MAX_RETRY; i++) {
			ret = ft5306_i2c_read_value(this_client, CHIP_ID_ADDR, &chip_id, 1);
		}
		if (ret) {
	...
	if(ret) {
		printk(KERN_ERR "leesh tsp get chip current Firmware version failed.\n");
		goto exit_init;
	}
	printk(KERN_ERR "Vulcan tsp chip_fw_ver 0x%2x  firmware_ver 0x%2x\n", chip_fw_ver, firmware_ver);

	ft5306_firmware_ver = chip_fw_ver;
	...
	if (chip_fw_ver < firmware_ver || chip_fw_ver == 0xa6) {
		...
		if (ret) {
			printk(KERN_ERR "leesh tsp upgrade TP firmware failed!\n");
		} else {
			msleep(10);
		}
		/*after updata need calibrate*/
		fts_register_write(0xfc, 4);
		ft5306_reset_func();//leesh add
		msleep(1000);
		ft5306_i2c_read_value(this_client, 0xA6, &chip_fw_ver, 1);
		ft5306_firmware_ver = chip_fw_ver;
		printk(KERN_ERR "the new firmware version is %x\n", chip_fw_ver);
	} else {
		printk(KERN_ERR "no need upgrade TSP firmware!\n");
	}
...
device_error_exit:
	ft5306_state = 0;	// by sinclair.lee 20121019 to avoid crash on no TSP
