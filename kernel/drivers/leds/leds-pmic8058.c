...
static int pmic8058_led_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < pdata->num_leds; i++) {
		...
		// BR.Hwang 2012-05-17 Fixed Cricket issue (Blue charging light doesn't stay on entire time of charging).
		if(PMIC8058_ID_LED_0 == led_dat->id)
		{
			led_dat->cdev.flags = 0;
		}

		printk( KERN_ERR "leds-pmic8058: led id(0x%X), led flags(0x%X)\n", led_dat->id, led_dat->cdev.flags);
