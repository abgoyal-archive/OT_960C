...
#define FEATURE_M7_DW9714_LSC_MODE			// kykim lsc : feature for LSC (linear Slope Control)
#undef FEATURE_M7_DW9714_DM_MODE			// kykim damping : feature for DM (Direct Method)
...
#define DW9714_T_SRC_LSB  0xC0
#define DW9714_T_SRC_LSB_MASK 0xF8
#define DW9714_PD_FLAG_MASK 0x00
#define DW9714_S4_S0_MODE_MASK 0x00
#ifdef FEATURE_M7_DW9714_LSC_MODE		// kykim lsc 
#define DW9714_S4_S0_1011_MASK 0x0B
#define DW9714_FLAG_MASK 0x4000
#endif
...
#ifdef FEATURE_M7_DW9714_LSC_MODE  // kykim af_i2c read 
static int ov5647_af_i2c_read(unsigned short raddr, unsigned short *rdata, int rlen)
{
	int32_t rc = 0;
	unsigned char buf[2]={0};//jerry,20101028
	struct i2c_msg msg[] = {
		{
		.addr  = raddr,
		.flags =  I2C_M_RD,
	...
	if (i2c_transfer(ov5647_client->adapter, msg, 1) < 0) {
		CDBG("KYKIM_ ov5647_AF_i2c_rxdata failed!\n");
		return -EIO;
	}
...
static int ov5647_write_exp_gain(uint16_t gain, uint32_t line){
	...
	rc = ov5647_i2c_write(ov5647_client->addr, 0x3208, 0, 2); // kykim aec : 
...
/*	
	...
	printk("ov5647_write_exp_gain: {0x350b,0x%2x}\n", debug_buf);
       printk("ov5647_write_exp_gain: <<<<<<<<<<<<<<<<\n");	
*/
	//group write end   // kykim aec : using group writing 
...
static int32_t ov5647_set_snapshot_exposure_gain(uint16_t gain, uint32_t line)
{
	...
	if((ov5647_ctrl->my_reg_gain - gain) > 30 /*goffset*/) // If the difference of gain is more than 30, than split it. 
	{  // decreasing 
		ov5647_ctrl->my_reg_gain = ov5647_ctrl->my_reg_gain - 3 /*goffset*/ ;
		line = (uint32_t)ov5647_ctrl->my_reg_line_count;
	} 	
	else if(( gain - ov5647_ctrl->my_reg_gain) >  30 /*goffset*/) // If the difference of gain is more than 30, than split it. 
	{  // increasing
		ov5647_ctrl->my_reg_gain = ov5647_ctrl->my_reg_gain + 3 /*goffset*/ ;
		line = (uint32_t)ov5647_ctrl->my_reg_line_count;
	}else{	
...
static int32_t ov5647_set_default_focus(void)
{
	...
	int 	rc = 0;
#ifdef FEATURE_M7_DW9714_DM_MODE	
	int		i =0; // kykim damping 
#endif
#define I2C_delay	1
	
	printk(KERN_INFO "ov5647_set_default_focus, curr = 0x%x", ov5647_ctrl->curr_lens_pos );
#ifdef FEATURE_M7_DW9714_DM_MODE   // SW damping to reducing glitch noise. // kykim damping
#define goffset  0x0a		// 0x1e => 0x14 => 0a
#define total_steps  60		// 20 => 30 => 60
	
for( i=0; i < total_steps ; i++){  // split max total_steps
	printk(KERN_INFO "ov5647_set_default_focus, curr = 0x%x", ov5647_ctrl->curr_lens_pos );
	if( ov5647_ctrl->curr_lens_pos > goffset /*goffset*/){ // larger than goffset, than split it. 
	  ov5647_ctrl->curr_lens_pos = ov5647_ctrl->curr_lens_pos - goffset /*goffset*/ ;	  
	}else{	// under goffset, just go as it was. 
		printk(KERN_INFO "ov5647_set_default_focus: Break " );
		break;
	}
	...
	printk(KERN_INFO "ov5647_set_default_focus: lsb = 0x%x, msb = 0x%x",code_val_lsb, code_val_msb );
	rc = ov5647_af_i2c_write(OV5647_AF_I2C_ADDR, code_val_msb, code_val_lsb, 1);
	mdelay(I2C_delay);
	if ( rc != 0){
		printk(KERN_INFO "ov5647_set_default_focus: ERR ~~ Break " );
		break; 
	}
}
#endif    // end of SW damping	
	...
	code_val_lsb = (uint8_t)((ov5647_ctrl->curr_lens_pos & 0x000F) << 4);
#ifdef FEATURE_M7_DW9714_LSC_MODE				
	code_val_lsb |= DW9714_S4_S0_1011_MASK;  // kykim lsc 
#else
	code_val_lsb |= DW9714_S4_S0_MODE_MASK;
#endif
	printk(KERN_INFO "ov5647_set_default_focus: lsb = 0x%x, msb = 0x%x",code_val_lsb, code_val_msb );
#ifdef FEATURE_M7_DW9714_LSC_MODE				
	{
		uint16_t  read_af_reg = 0xFFFF;
		int cnt = 0; 
		for(cnt = 0; cnt < 100; cnt++){
			rc = ov5647_af_i2c_read(OV5647_AF_I2C_ADDR, &read_af_reg, 2);
	if ( rc != 0) {
				printk(KERN_ERR "KYKIM_ I2C read err" );
				mdelay(2);	// delay 
				continue;
	}
			//printk(KERN_ERR "KYKIM_ read_af_reg = 0x%x",read_af_reg );
			read_af_reg &= DW9714_FLAG_MASK; 
			if(read_af_reg != DW9714_FLAG_MASK){
				printk(KERN_ERR "KYKIM_ read_af_reg = 0x%x",read_af_reg );
				break; 
	}
			mdelay(2);	// delay 
...
#define I2C_delay	1
	...
	printk(KERN_INFO "ov5647_move_focus: requested position =0x%x",next_position );  // kykim log 
	if(next_position < 0) {
...
#ifdef FEATURE_M7_DW9714_DM_MODE		 // kykim damping 
#define goffset  0x0a		// 0x1e => 0x14 => 0a
#define total_steps  60		// 20 => 30 => 60

	for( i=0; i < total_steps ; i++){  // split max 20 times
	printk(KERN_INFO "ov5647_move_focus, old = 0x%x, new = 0x%x", ov5647_ctrl->curr_lens_pos, next_position );
		if(( ov5647_ctrl->curr_lens_pos - next_position) > goffset /*goffset*/) //
		{  // decreasing 
		  ov5647_ctrl->curr_lens_pos = ov5647_ctrl->curr_lens_pos - goffset /*goffset*/ ;
		}	  
		else if((next_position - ov5647_ctrl->curr_lens_pos) > goffset /*goffset*/) // 
		{  // increasing
		  ov5647_ctrl->curr_lens_pos = ov5647_ctrl->curr_lens_pos + goffset /*goffset*/ ;
			...
			code_val_lsb = (uint8_t)((ov5647_ctrl->curr_lens_pos & 0x000F) << 4);
			code_val_lsb |= DW9714_S4_S0_MODE_MASK;
			printk(KERN_INFO "ov5647_move_focus: lsb = 0x%x, msb = 0x%x",code_val_lsb, code_val_msb );
			rc = ov5647_af_i2c_write(OV5647_AF_I2C_ADDR, code_val_msb, code_val_lsb, 1);
			break;
		}else{  // under 30, just go as it was. 
			ov5647_ctrl->curr_lens_pos = next_position; 
			code_val_msb = (uint8_t)((ov5647_ctrl->curr_lens_pos & 0x03FF) >> 4);
			code_val_lsb = (uint8_t)((ov5647_ctrl->curr_lens_pos & 0x000F) << 4);
			code_val_lsb |= DW9714_S4_S0_MODE_MASK;
...
#else	
	ov5647_ctrl->curr_lens_pos = next_position; 
	printk(KERN_INFO "ov5647_move_focus: Next position=0x%x",next_position );  // kykim log 
	...
	if ( rc != 0) {
		...
		mdelay(I2C_delay); //delay may set based on the steps moved when I2C write successful.
	}
	return 0;
}
...
{
	printk(KERN_ERR "--CAMERA--ov5647_probe_init_done !!\n");

	gpio_direction_output(data->sensor_pwd,  1);
	//shut down sub camera   // kykim vreg 2
	gpio_set_value(33, 1);
	mdelay(20);
...
static int ov5647_probe_init_sensor(const struct msm_camera_sensor_info *data)
{
	...
	gpio_set_value(data->sensor_reset, 1);  // kykim vreg 
...
detect_retry:
	...
	return 0;
init_probe_fail:
	ov5647_probe_init_done(data);  // kykim vreg 
	return rc;
}
...
static int ov5647_setting(enum msm_s_reg_update rupdate,enum msm_s_setting rt)
{
	...
	switch (rupdate)	{
		case S_UPDATE_PERIODIC:
			{
				ov5647_stop_stream();
				//kykim shutterlag msleep(100);
				msleep(20);
                                
				// kykim tp1 // kykim test if (OV5647_CSI_CONFIG == 0)
				{          
					struct msm_camera_csi_params ov5647_csi_params;                                       
#ifdef FIXED_CAMIF_RECOVERY	// kykim tp1 
					printk(KERN_INFO "%s KYKIM]] CSI reset and config \n", __func__);
 					msm_camio_csi_reset(pldev);
                                        msleep(10);
#endif
					ov5647_csi_params.lane_cnt = 2;
					ov5647_csi_params.data_format = CSI_10BIT; // kykim rawsnapshot CSI_8BIT;
					ov5647_csi_params.lane_assign = 0xe4;
					ov5647_csi_params.dpcm_scheme = 0;
					ov5647_csi_params.settle_cnt = 0x14;  // //0x14;  // kykim tp1 0x18 -> 7 
				...
				{ 
					printk(KERN_ERR "--CAMERA-- OV5647_RES_PREVIEW (Start)\n");  // kykim tp1
		...
		case S_REG_INIT:
			{
				OV5647_CSI_CONFIG = 0;
				printk(KERN_ERR "--CAMERA-- S_REG_RESET First \n");  // kykim reg_reset 
				if (ov5647_i2c_write(ov5647_client->addr, 0x0103, 0x01, 1)){
					printk(KERN_ERR "%s failed\n", __func__);
				}
				msleep(10);
...
static int32_t ov5647_poweron_af(void)
{
	...
	mdelay(12);
#if 1  //def FEATURE_M7_DW9714_LSC_MODE kykim lsc :: leave this code for later use. 
// Ringing setting ON : 
	printk(KERN_INFO "SET LSE REG for AF " );  // kykim log 
...
static int ov5647_sensor_release(void)
{
	...
	// kykim vreg 2 : added this for hw standby mode 
	printk(KERN_ERR "--CAMERA-- Standby set MIPI reg first \n"); 
	if (ov5647_i2c_write(ov5647_client->addr, 0x3018, 0x47, 1)){
		printk(KERN_ERR "%s failed\n", __func__);
	}
	msleep(10);
...
static int ov5647_sensor_config(void __user *argp)
{
	...
	switch (cdata.cfgtype)
	{
		...
		case CFG_SET_FPS:
			printk(KERN_ERR "--CAMERA-- %s: KYKIM]] set fps : request fps=%u, fps_div=%u, pict_fps_div=%u \n"
				,__func__, cdata.cfg.fps.request_fps, cdata.cfg.fps.fps_div , cdata.cfg.fps.pict_fps_div);
			if(cdata.cfg.fps.request_fps == 8){ // kykim 8fps : set 8fps video recording for mms iot			
				int len;
				int i = 0;
				ov5647_stop_stream();
				msleep(100);								
...
#endif
					ov5647_csi_params.lane_cnt = 2;
					ov5647_csi_params.data_format = CSI_10BIT; // kykim rawsnapshot CSI_8BIT;CSI_10BIT
					ov5647_csi_params.lane_assign = 0xe4;
					ov5647_csi_params.dpcm_scheme = 0;
					ov5647_csi_params.settle_cnt = 0x14;  //0x14;  // kykim tp1 0x18 -> 7 
					printk(KERN_INFO "%s call msm_camio_csi_config\n", __func__);
				...
				printk(KERN_ERR "--CAMERA-- OV5647_RES_8 FPS PREVIEW (Start)\n"); 
				len = sizeof (ov5647_preview_8fps_array) /sizeof (ov5647_preview_8fps_array[0]);
				for(i = 0; i < len; i++){
					rc = ov5647_i2c_write(ov5647_client->addr, \
						ov5647_preview_8fps_array[i].register_address, ov5647_preview_8fps_array[i].register_value,10);
				}
				msleep(20);
				ov5647_start_stream();
			}
		...
		case CFG_SET_PICT_FPS:
			...
			break;
		case CFG_SEND_SENSOR_RESET :    //32   kykim camreset			
			printk(KERN_ERR "--CAMERA-- %s: SENSOR reset Command=%d !\n",__func__,cdata.cfgtype);
			rc = 0;
			gpio_set_value(ov5647_ctrl->sensordata->sensor_reset, 0);
			mdelay(1);  //greater than 1ms 
			gpio_set_value(ov5647_ctrl->sensordata->sensor_reset, 1);
			break;
		default:
			printk(KERN_ERR "--CAMERA-- %s: Command=%d (Not Implement)!!\n",__func__,cdata.cfgtype);
...
static int ov5647_sensor_probe(const struct msm_camera_sensor_info *info,struct msm_sensor_ctrl *s)
{
	...
	s->s_mount_angle = 0;
	printk(KERN_ERR "--CAMERA-- %s (End...)\n",__func__);
	goto probe_done;  // kykim vreg : 

probe_fail:
	printk(KERN_ERR "--CAMERA-- %s fail! \n", __func__);
