...
#define DBG_OUT_RC
#endif
// kykim module : these are for module build 
//#define msm_camio_camif_pad_reg_reset	((void(*)())0xc03d6dac)
//#define msm_camio_clk_rate_set	( (void(*)(int)) 0xc03d6e4c   )
...
{// kykim module
DBG_IN
#if 0	// kykim subcamera : i2c probe error fix
...
static int32_t ov7690_probe_init_done(const struct msm_camera_sensor_info *data)
{
	...
	// main camera pwdn set high  // kykim vreg 2
...
detect_retry_msb:
	...
	DBG(" ov7690 read model ID MSB %d chance left \n", probe_retry); // kykim test 
	...
	if (rc < 0){
		if(probe_retry){
			...
			//msm_camera_vreg_reset();
			mdelay(3);
			gpio_set_value(data->sensor_pwd, 0);  // 
			//msm_camio_clk_enable(CAMIO_CAM_MCLK_CLK);

			mdelay(20);
			//gpio_set_value(data->sensor_pwd, 0);  // kykim test 
			goto detect_retry_msb;
		}
		goto init_probe_fail;
        }
	probe_retry = 3; // give 3 chance to read here ~~ 
detect_retry_lsb:
	DBG(" ov7690 read model ID LSB %d chance left \n", probe_retry);
	rc = ov7690_i2c_read(REG_OV7690_MODEL_ID_LSB, &model_id_lsb, 1);
	if (rc < 0){
		if(probe_retry){
			probe_retry--;
			goto detect_retry_lsb;
		}
		goto init_probe_fail;
        }
...
DBG_IN
	...
	case CFG_PWR_DOWN:
		...
		break;
// kykim 8fps VGA : Added set fps config. 		
		case CFG_SET_FPS:
			...
			if(cdata.cfg.fps.request_fps == 8){ // kykim 8fps : set 8fps video recording for mms iot			
				int32_t rc = 0;
				printk(KERN_ERR "--CAMERA-- OV7690 SET 8 FPS PREVIEW\n"); 
				
				rc = ov7690_i2c_write_table(&fps_tbl_8[0], ARRAY_SIZE(fps_tbl_8));
				DBG("write 8fps code here~~ rc =%d", rc);
				if (rc < 0){
					return rc;
				}				
				//mmsfps_mode = 1; 
...
DBG_IN
	...
	/*power down active high with internel pull down resistor
	PWDN should be connected to ground outside of module if unused*/
	gpio_direction_output(ov7690_ctrl->sensordata->sensor_pwd, 1);  // kykim vreg 2
	
// main camera pwdn set high 
	gpio_direction_output(143, 1);
DBG_OUT_RC
	return rc;
}
...
DBG_IN
	...
	s->s_mount_angle = 270;  //?? // kykim 90degree   0 => 90 -> 270  // kykim rotation 
	DBG("%s probe success!\n", __func__);
	goto probe_done;
...
// kykim module build [[[
static void __exit ov7690_exit(void){
	return platform_driver_unregister(&msm_camera_ov7690_driver);
}
#ifdef MODULE
EXPORT_SYMBOL(ov7690_init);
EXPORT_SYMBOL(ov7690_exit);
#endif

module_init(ov7690_init);
module_exit(ov7690_exit);
// ]]] end of module build 
MODULE_DESCRIPTION("OMNI VGA YUV sensor driver");
// kykim module : MODULE_LICENSE("GPL V2");
MODULE_LICENSE("GPL");
