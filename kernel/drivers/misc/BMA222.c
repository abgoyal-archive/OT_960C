...
//leesh test
	...
	printk(KERN_ERR "bma222_read_accel_xyz = (%d)(%d)(%d).\n", acc->x, acc->y, acc->z);
	printk(KERN_ERR "bma222_read_new_data = (%d)(%d)(%d).\n", data[0], data[2], data[4]);

	comres = p_bma222->bma222_BUS_READ_FUNC(p_bma222->dev_addr, bma222_STATUS1_REG, data, 2);
	printk(KERN_ERR "bma222_read_status 1 2 = (%d)(%d).\n", data[0], data[1]);

	comres = p_bma222->bma222_BUS_READ_FUNC(p_bma222->dev_addr, bma222_OFFSET_CTRL_REG, data, 6);
	printk(KERN_ERR "bma222_read_status 36  = (%d)(%d)(%d)(%d)(%d)(%d).\n", data[0], data[1], data[2], data[3], data[4], data[5]);
//leesh test
#endif
      }
   return comres;
...
int bma222_get_ee_w(unsigned char *eew)
   ...
   if (p_bma222==C_Zero_U8X)
      ...
      {
      comres = p_bma222->bma222_BUS_READ_FUNC(p_bma222->dev_addr, bma222_EE_WRITE_SETTING_S__REG,eew, C_One_U8X);
