...
// FEATURE_M7LAB for GB argument(kind) deleted
static int bma222_driver_detect(struct i2c_client *client,
...
static int bma222_driver_probe(struct i2c_client *client,
			 const struct i2c_device_id *id)
{
	...
	tempvalue = i2c_smbus_read_word_data(client, 0x00);	// 20120926 to support slate DC test for later version than ES board
#else
	i2c_master_send(client, (char*)&tempvalue, 1);
	i2c_master_recv(client, (char*)&tempvalue, 1);

	i2c_master_send(client, (char*)&tempvalue, 1);	// 20120926 to support slate DC test for later version than ES board
	i2c_master_recv(client, (char*)&tempvalue, 1);	// 20120926 to support slate DC test for later version than ES board
	printk(KERN_INFO "leesh i2c_check_functionality error1 = (%d)\n", tempvalue);

	mdelay(1);
	tempvalue = 0;
	i2c_master_send(client, (char*)&tempvalue, 1);
	i2c_master_recv(client, (char*)&tempvalue, 1);
	printk(KERN_INFO "leesh i2c_check_functionality error2 = (%d)\n", tempvalue);

	mdelay(1);
	tempvalue = 0;
	i2c_master_send(client, (char*)&tempvalue, 1);
	i2c_master_recv(client, (char*)&tempvalue, 1);
	printk(KERN_INFO "leesh i2c_check_functionality error3 = (%d)\n", tempvalue);
#endif
	...
	bma222_set_bandwidth(1);		//bandwidth 250Hz
	bma222_set_range(0);			//range +/-2G

	bma222_set_int_set(0, 0);
...
#endif
static int bma222_driver_suspend(struct i2c_client *client,pm_message_t mesg)
{
#ifdef BMA222_DEBUG
    printk(KERN_INFO "%s\n",__FUNCTION__);
#endif	
...
static int bma222_driver_resume(struct i2c_client *client)
{
#ifdef BMA222_DEBUG
    printk(KERN_INFO "%s\n",__FUNCTION__);
#endif	
...
static int bma222_driver_remove(struct i2c_client *client)
{
...
// FEATURE_M7LAB for GB address_data -> address_list & its contents
static struct i2c_driver bma222_driver = {	
	...
	.probe		= bma222_driver_probe,//bma222_probe,
	.remove		= bma222_driver_remove,//bma222_remove,
	.detect		= bma222_driver_detect,//bma222_detect,
    .suspend    = bma222_driver_suspend,//bma222_suspend,
    .resume     = bma222_driver_resume,//bma222_resume,
};

static int __init BMA222_driver_init(void)
{
#ifdef BMA222_DEBUG
	printk(KERN_INFO "%s\n",__FUNCTION__);
#endif
...
static void __exit BMA222_driver_exit(void)
{
...
module_init(BMA222_driver_init);
module_exit(BMA222_driver_exit);
