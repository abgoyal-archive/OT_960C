...
**********************************************/
#if 0
static int bma222_init(void *mlsl_handle, struct ext_slave_descr *slave,
					struct ext_slave_platform_data *pdata)
{
	int result = ML_SUCCESS;
	unsigned char data = 0;
	u8 reg = 0x00;	//device id
	result = sensor_i2c_read((struct i2c_adapter *) mlsl_handle,
					pdata->address, reg,
					1, &data);
	printk(KERN_ERR"leesh hahaha bma222 devide id=%x",data);
	if (ML_SUCCESS != result || data != 0x03){
		pr_err("%s read error\n", __func__);
		return -1;
	}else{
		pr_debug("%s read ok\n", __func__);
...
static int bma222_exit(void *mlsl_handle, struct ext_slave_descr *slave,
					struct ext_slave_platform_data *pdata)
{
	return 0;
}
