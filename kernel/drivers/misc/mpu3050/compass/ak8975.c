...
#if 0
int ak8975_init(void *mlsl_handle, struct ext_slave_descr *slave,
					struct ext_slave_platform_data *pdata)
{
	...
	result = sensor_i2c_read((struct i2c_adapter *) mlsl_handle,
				    pdata->address, reg,
				    1, &data);
	printk(KERN_ERR"leesh hahaha ak8975 devide id=%x",data);
	if (ML_SUCCESS != result || data != 0x48){
		pr_err("%s read error\n", __func__);
		return -1;
	}else{
		pr_debug("%s read ok\n", __func__);
...
int ak8975_exit(void *mlsl_handle, struct ext_slave_descr *slave,
					struct ext_slave_platform_data *pdata)
{
	return 0;
}
