...
/* change the order for ledtorch by shane.choi [Start] */
#define FLASH_MODE        	4

#define TORCH_MODE_SLIGHT    	1 // 3
#define TORCH_MODE_NORMAL  	2
#define TORCH_MODE_HIGH        	3 // 1
#define TORCH_FLASH_OFF   	0
/* change the order for ledtorch by shane.choi [End] */

#define PWM_PERIOD_DEF  	80
#define PWM_DUTY_DEF       	7

#define PWM_PERIOD_SLIGHT		280
#define PWM_DUTY_SLIGHT			70

#define PWM_PERIOD_NORMAL		60  //kykim highfreq 280
#define PWM_DUTY_NORMAL			60  //140

#define PWM_PERIOD_HIGH			280
#define PWM_DUTY_HIGH			280
...
static struct pm8058_gpio flash_drv_enable = {
...
static struct pm8058_gpio torch_flash_enable = {
	.direction      = PM_GPIO_DIR_OUT,
	.pull           = PM_GPIO_PULL_NO,
	.out_strength   = PM_GPIO_STRENGTH_HIGH,
	.function       = PM_GPIO_FUNC_NORMAL,
...
int torch_flash_control(int mode)
{
	...
	switch (mode) {
		case TORCH_MODE_SLIGHT:
			pr_info("%s: torch  slightly light = %d(%d)\n", __func__, PWM_DUTY_SLIGHT, PWM_PERIOD_SLIGHT);
			gpio_set_value_cansleep(PM8058_GPIO_PM_TO_SYS(TORCH_FLASH), 0);
			gpio_set_value_cansleep(PM8058_GPIO_PM_TO_SYS(CAM_FLASH_DRV_EN), 0);
		...
		case FLASH_MODE:
			...
			gpio_set_value_cansleep(PM8058_GPIO_PM_TO_SYS(TORCH_FLASH), 0);
			gpio_set_value_cansleep(PM8058_GPIO_PM_TO_SYS(CAM_FLASH_DRV_EN), 1);
			udelay(50);
			gpio_set_value_cansleep(PM8058_GPIO_PM_TO_SYS(TORCH_FLASH), 1);
			break;
...
static int torch_flash_probe(struct platform_device *pdev)
{
	...
	ret = pm8058_gpio_config(TORCH_FLASH, &torch_flash_enable);
	...
	ret = pm8058_gpio_config(CAM_FLASH_DRV_EN, &flash_drv_enable);
	if (ret) {
