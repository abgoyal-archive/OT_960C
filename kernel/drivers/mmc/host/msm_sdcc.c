...
#if 1	//	Teddy 2012-08-24 code aurora patch : Fix deadlock while resuming sdcc from userspace
...
msmsdcc_probe(struct platform_device *pdev)
{
	...
	pr_info("%s: Qualcomm MSM SDCC-core %d, at 0x%016llx irq %d,%d dma %d"
		" dmacrcri %d\n", mmc_hostname(mmc),
		pdev->id,
...
//Hugh add for suspend sdio card 1107 ++++
...
msmsdcc_runtime_resume(struct device *dev)
{
	...
	if (mmc) {
		...
		//mmc_resume_host(mmc);
//Hugh add for resume sdio card 1107 ++++
 		if(!mmc->card || (mmc->card && (mmc->card->type != MMC_TYPE_SDIO)))
	        {
			printk("###### Hugh enter[%s] mmc_suspend_host\n",__func__);
